...
/**
 * This class provides some special methods for BigInteger arithmetic that are useful for certain
...
public class ProtocolUtils extends BigIntegerUtil {
  ...
  /**
   * Hashes an identification string I and an integer j to a BigInteger of [0, max - 1], i.e. [0,
   * 2^(max.bitLength () - 1) - 1] where max / 2 < 2^(max.bitLength () - 1) = 2^( [lb(max)] ) <= max
   * 
   * @param I
  ...
  {
    return (new BigInteger(max.bitLength(), new Random((long) (I + String.valueOf(j)).hashCode()))).remainder(max);
  }

  /**
   * This routine yields a solution to the Chinese Remainder Theorem (CRT), as described in
   * Algorithmus 3.4. This special case describes the application with t=2.
   * 
   * @param d1
   *          a BigInteger
   * @param d2
  ...
  /**
   * See ALGORITHMS 5.2.1. Calculate the square root of <code>val</code> modulo <code>modulus</code>
   * . <code>modulus</code> must be a prime number, and <code>val</code> a quadratic remainder.
   * Then, an x out of [1, ..., modulus-1] is calculated with x**2 mod <code>modulus</code> = a mod
   * <code>modulus</code>.
   * 
   * @param val
  ...
  /*
   * Die folgende Routine ist ein Teil von Algorithmus 5.2.1. Die Zahl p wird als ungerade
   * vorausgesetzt. Der Algorithmus isoliert die ungeraden Faktoren. (u.a.) Die Endbedingung ist:
   * p-1 = m*2^l, l maximal.
   */
  ...
  /*
   * Diese Routine ist ein Teil von Algorithmus 5.2.1. Sie liefert: min { k >= 0 | y^((2^k)*m) mod p
   * = 1 }
   */
