...
/**
 * Die Klasse ServerThread uebernimmt eine Clientverbindung, nachdem diese durch den Server
 * initialisiert worden ist, so dass dieser fuer neue Verbindungen wieder zur Verfuegung steht. Sie
 * leitet Nachrichten an den Server weiter, der diese dann (ueber einen weiteren ServerThread) an
 * den entsprechenden Ziel-Client sendet.
 * 
...
public class ServerThread extends Thread {
  ...
  public ServerThread(Socket connection, Server server, int cnt, int gameID, String name) {
    ...
    try {
      out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(this.connection.getOutputStream())));
  ...
  public String receive() throws InterruptedIOException, IOException {
    ...
    /*
     * while (s.equals("")) { while(in.ready()) { result.print(in.readLine()); if (in.ready())
     * result.println(); } result.flush(); w.flush(); s = w.toString(); }
     */
  ...
  /**
   * Sendet den String "message" ueber die Socket-Verbindung an das Communicator- Objekt des
   * Clients.
   * 
   * @param message
