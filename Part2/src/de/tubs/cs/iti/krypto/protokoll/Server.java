...
/**
 * Die Klasse Server implementiert ein eigenstaendiges Programm, das auf einem Rechner gestartet
 * wird, dessen Adresse den Spielern bekannt ist. Es soll bis zu 50 Verbindungen mit Clients
 * annehmen und die auf diesen laufenden Spielprotokolle verwalten koennen.
 * 
...
public class Server {
  ...
  /**
   * Konstruktor, initialisiert die Variable servSocket durch Erzeugen eines ServerSocket-Objekts
   * auf dem angegebenen Port. Desweiteren wird der Vektor "games" initialisiert, der die Spiele vom
   * Typ Game enthalten wird.
   * 
   * @param port
   *          Portnummer
   */
  ...
  /**
   * Diese Methode dient zum Aufnehmen eines neuen Spielers. Sie verarbeitet die in "data"
   * enthaltenen Spielerdaten (Name des Spiels, minimale und maximale Spieleranzahl des Spiels) und
   * entscheidet dann, ob der Spieler in ein bestehendes Spiel vom Typ Game hinzugefuegt werden
   * kann, oder ob ein neues Objekt vom Typ "Game" erzeugt werden muss. Dann wird ein eigener Thread
   * fuer den neuen Spieler erzeugt, der in der "players" Liste des jew. Game-Objekts abgespeichert
   * wird.
   * 
   * @param data
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (game.getNameOfTheGame().equals(name)) {
          if (game.is_running || game.isFull()) {
            ...
            ServerThread thread = new ServerThread(this.connection, this, game.getCurrentPlayers(), game.getGameID(), game.getNameOfTheGame());
            thread.start();
            game.add(thread);
  ...
  /**
   * Diese Methode loescht einen Spieler aus einem Spiel, wenn dieser z.B. seinen Client vor Beginn
   * des Spiels beendet hat. Dazu wird die delete-Methode des entsprechenden Game-Objekts
   * aufgerufen.
   * 
   * @param data
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (game.getNameOfTheGame().equals(name) && gameID == game.getGameID()) {
          ...
          if (game.getPlayers().size() == 0) {
            games.remove(i);
            System.out.println("server: game " + (i + 1) + " removed from list");
          }
        }
      }
    } catch (NoSuchElementException e) {
  ...
  /**
   * Diese Methode dient zum Weiterleiten von Nachrichten zum Zielspieler, der im String "data"
   * enthalten ist. Sie wird vom sendenden ServerThread aufgerufen und schickt die Nachricht ueber
   * den Ziel-ServerThread an das entsprechende Communicator-Objekt des Empfaengers. Die Variable
   * "gameID" (s. Klasse ServerThread) dient der Methode zur Unterscheidung von mehreren Spielen des
   * gleichen Typs. Ausserdem werden Meldungen, die an den Server direkt gehen, an die
   * entsprechenden Methoden des Servers weitergeleitet ("Spiel starten" und "Client beendet").
   * Diese Meldungen erkennt die Methode daran, dass die Zieladresse im String "data" (normalerweise
   * die Spielernummer des Empfaengers) negativ ist.
   * 
   * @param gameID
   *          Spiel-ID
   * @param data
  ...
  public synchronized void forward(int gameID, String data) {
    ...
    try {
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (game.getNameOfTheGame().equals(name) && gameID == game.getGameID()) {
          if (target != -1) {
            ServerThread recipient = ((ServerThread) game.getPlayers().elementAt(target));
  ...
  /**
   * Diese Methode speichert die Nachrichten, die der Server ausgibt in einer Log-Datei ab.
   * 
   * @param message
  ...
  /**
   * In dieser Methode wird eine neue Instanz vom Typ Server erzeugt. Danach werden innerhalb einer
   * Endlosschleife Verbindungen angenommen, denen von der Methode addPlayer ein eigener Thread
   * zugewiesen wird und die in der Liste "games" verwaltet werden.
   * 
   * @param args
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (true) {
        ...
        serv.in = new BufferedReader(new InputStreamReader(serv.connection.getInputStream()));
  ...
  /**
   * Diese Methode empfaengt die Spieldaten, die der Client bei der Anmeldung an den Server sendet.
   * 
  ...
  /**
   * Diese Methode unterbricht die Verbindung zu den Spielern eines Spiels. Sie wird vom
   * ServerThread des Spielers aufgerufen, dessen Client die Verbindung als erster unterbrochen hat.
   * Nach Aufruf der Methode shutdown() in Klasse Game wird dann das Spiel ggf. aus der Liste
   * geloescht.
   * 
   * @param data
